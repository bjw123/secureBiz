...
def startQuestionnaire():
    '''Navigate to Configure Macro questionnaire.'''
    wait(1)
    click("assess.png")
    wait(1)
    click("macro_settings.png")
...
def readResult():
    ...
    if exists(Pattern("three.png").similar(0.90)):
        result = '3'
    elif exists (Pattern("two.png").similar(0.85)):
        result = '2'
...
def generateTestCases():
    ...
    for tuples in range(4):
        ...
        ("trusted_locations.png",) + buttonsOne[buttonsOneIndex]
        buttonsOneIndex += 1
        
    buttonsTwoIndex = 0
    for tuples in range(4):
        buttonsTwo[buttonsTwoIndex] = buttonsTwo[buttonsTwoIndex][:0] + \
        ("trusted_publishers.png",) + buttonsTwo[buttonsTwoIndex]
        buttonsTwoIndex += 1
    
    buttonsThreeIndex = 0
    for tuples in range(4):
        buttonsThree[buttonsThreeIndex] = buttonsThree[buttonsThreeIndex][:0] + \
        ("user_approval.png",) + buttonsThree[buttonsThreeIndex]
        buttonsThreeIndex += 1
 
    buttonsFourIndex = 0
    for tuples in range(4):
        buttonsFour[buttonsFourIndex] = buttonsFour[buttonsFourIndex][:0] + \
        ("none.png",) + buttonsFour[buttonsFourIndex]
        buttonsFourIndex += 1
    ...
    for tests in range (noTestCases):
        ...
        for noQuestions in range (3):
            ...
            if answer[answerIndex] == 'yes.png':
                ...
                currentTest.append('No')
            elif answer[answerIndex] == 'trusted_locations.png':
                currentTest.append('A')
            elif answer[answerIndex] == 'trusted_publishers.png':
                currentTest.append('B')
            elif answer[answerIndex] == 'user_approval.png':
