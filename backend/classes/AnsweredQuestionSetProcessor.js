...
const AnsweredQuestionSetProcessor = class AnsweredQuestionSetProcessor {
  ...
  processQuestionCategory = async (category) => {
    ...
    const categoryData = await models.connectDb().then(() => (models.Category.findOne(query)))
    ...
    const categoryQuestionsAnswered = this.QuestionsAnswered.filter(qa => qa.QuestionCategory === category)

    // This will give you the final level in number form.
    this.Level = this.evaluateConditionsScore(categoryQuestionsAnswered, CalculationRules, category)

    return this.QuestionCategory = categoryData
  }
  ...
  filterQuestion = (QuestionSetNumber) => {
    ...
    const filteredQuestionsAnswered = QuestionsAnswered.filter(qa => qa.QuestionSetNumber === QuestionSetNumber)
    ...
    const emptyAnsweredQuestions = new QuestionAnswered(this.Category, 'DUMMY', '', 0, 0, 0, 0, [])
